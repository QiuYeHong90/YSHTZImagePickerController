...
/**
 根据图片数组获取每个图片大小
 maxByte 如果maxByte <=0 默认20MB = 1024*1024*20
 @param photos TZAssetModel 对象数组
 @param maxByte 每张图片最大字节 例如20MB 1024*1024*20
 @param completion 成功 返回数组总共多少兆
 @param faile 失败提示
 */
- (void)dy_getPhotosBytesWithArray:(NSArray *)photos maxByte:(NSInteger)maxByte completion:(void (^)(NSString *totalBytes))completion faile:(void (^)(void))faile {
    NSInteger max_byte ;
    if (maxByte <= 0) {
        max_byte = 1024*1024*20 ;
    }else{
        max_byte = maxByte ;
    }
    if (!photos || !photos.count) {
